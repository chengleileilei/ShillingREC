...
from rectool import pick_optim, VarDim, getPopularItemId


def RevAdv_attack_loss(logits, labels):
...
class RevAdv(BaseAttacker):
    def __init__(self, train_df,config) -> None:
        super().__init__(train_df,config)
        selected_item_num = config['attack_model_p']['selected_item_num'] # 暂未使用
        self.selected_ids = getPopularItemId(self.train_df[self._ITEM], selected_item_num, self.target_id_list)
        self.attack_num = self.attacker_num

        self.n_users = self.dataset.n_users
        self.n_items = self.dataset.n_items

        self.device = config['device']
        self.train_array = self.dataset.to_matrix()
        self.surrogate = config['attack_model_p']["surrogate_model"]
        self.config = config
        self.build_network()
    ...
    def build_network(self):
        ...
        self.netG = RecsysGenerator(self.device, self.config['task_type'], self.real_template).to(self.device)

        self.G_optimizer = pick_optim(self.config['attack_model_p']['optim_g'])(
            self.parameters(), lr=self.config['attack_model_p']['lr_g']
        )
    ...
    def train_step(self, **config):
        ...
        for target_id in target_id_list:
            target_users = np.where(self.train_array[:, target_id] == 0)[0] # 选择对target_id没有评分的用户
            attack_target = np.zeros((len(target_users), self.n_items)) # 生成攻击矩阵
            attack_target[:, target_id] = 1.0  # 为target_id的位置赋值1
            attack_target = (
    ...
    def get_explicit_fake_df(self):
        ...
        print("fake_tensor", fake_tensor)

        fake_profiles = fake_tensor.detach().cpu().numpy()
    ...
    def get_sur_predictions(self, fake_tensor):
        ...
        if surrogate == 'WMF':
            sur_trainer_ = WMFTrainer(
                ...
                hidden_dim=self.config['attack_model_p']['hidden_dim_s'],
                # device=self.device,
                device=self.device,
                lr=self.config['attack_model_p']['lr_s'],
                weight_decay=self.config['attack_model_p']['weight_decay_s'],
                batch_size=self.config['attack_model_p']['batch_size_s'],
                weight_pos=self.config['attack_model_p']['weight_pos_s'],
                weight_neg=self.config['attack_model_p']['weight_neg_s'],
                verbose=False,
            )
            epoch_num_ = self.config['attack_model_p']['epoch_s']
            unroll_steps_ = self.config['attack_model_p']['unroll_steps_s']
        else:
            raise ValueError(
    ...
    def fit_adv(self, data_tensor, epoch_num, unroll_steps):
        ...
        for i in range(1, epoch_num - unroll_steps + 1):
            ...
            for batch_idx in self.minibatch(idx_list, batch_size=self.batch_size):
                ...
                loss.backward()
                self.optimizer.step()
            print('weighted_mse_loss-Epoch:', i, 'Loss:', epoch_loss)
...
class BaseGenerator(nn.Module):
    def __init__(self, device, task_type, input_dim):
    ...
    def project(self, fake_tensor):
        """
        对输入的fake_tensor进行了四舍五入，并将所有小于0的元素替换为0，所有大于5的元素替换为5。
        """
        ...
        if self.task_type == 'rating':
            ...
            fake_tensor.data = torch.where(
                fake_tensor < 0.1, torch.zeros_like(fake_tensor).to(self.device), fake_tensor
            )
            fake_tensor.data = torch.where(
                fake_tensor > 0.1, torch.tensor(1.0).to(self.device), fake_tensor
            )
        return fake_tensor
...
class RecsysGenerator(BaseGenerator):
    def __init__(self, device, task_type, init_tensor):
        super(RecsysGenerator, self).__init__(device, task_type, init_tensor.shape[1])
        """
        fake_parameter
        """
