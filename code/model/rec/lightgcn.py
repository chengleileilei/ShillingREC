...
class LightGCN(AbstractRecommender):
    ...
    def calculate_rank_loss(self, bat_users,bat_pos_items,bat_neg_items):
        ...
        user = bat_users
        pos_item = bat_pos_items
        neg_item = bat_neg_items
    ...
    def calculate_rating_loss(self, bat_users,bat_items,bat_ratings):
        ...
        user_all_embeddings, item_all_embeddings = self.forward()
        u_embeddings = user_all_embeddings[bat_users]
        i_embeddings = item_all_embeddings[bat_items]

        # calculate BPR Loss
        pre_rating = torch.mul(u_embeddings, i_embeddings).sum(dim=1)
        mf_loss = torch.nn.MSELoss()(pre_rating,bat_ratings)

        # calculate regularization Loss
        u_ego_embeddings = self.user_embedding(bat_users)
        i_ego_embeddings = self.item_embedding(bat_items)
