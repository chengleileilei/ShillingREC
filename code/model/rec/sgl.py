...
class SGL(AbstractRecommender):
    ...
    def calculate_rank_loss(self, bat_users,bat_pos_items,bat_neg_items):
        ...
        user_list = bat_users
        pos_item_list = bat_pos_items
        neg_item_list = bat_neg_items
    ...
    def calculate_rating_loss(self, bat_users,bat_items,bat_ratings):
        ...
        user_sub1, item_sub1 = self.forward(self.sub_graph1)
        user_sub2, item_sub2 = self.forward(self.sub_graph2)
        mse_loss = self.calc_mse_loss(user_emd, item_emd, bat_users, bat_items, bat_ratings)
        ssl_loss = self.calc_ssl_loss(bat_users,bat_items,user_sub1, user_sub2, item_sub1, item_sub2)
        # print('mse_loss：{}，ssl_loss{}'.format(mse_loss,ssl_loss*self.ssl_weight_rating))
        total_loss = mse_loss + ssl_loss * self.ssl_weight_rating
        # total_loss = self.calc_mse_loss(user_emd, item_emd, bat_users, bat_items, bat_ratings) + self.ssl_weight_rating * self.calc_ssl_loss(bat_users,bat_items,user_sub1, user_sub2, item_sub1, item_sub2)
    ...
    def calc_bpr_loss(
        ...
        # print('bpr_loss：{}，reg_loss{}'.format(l1,l2 * self.reg_weight))

        return l1 + l2 * self.reg_weight
    
    def calc_mse_loss(self, all_user_emd, all_item_emd, bat_users, bat_items, bat_ratings):
        bat_user_emd = all_user_emd[bat_users]
        bat_item_emd = all_item_emd[bat_items]
        pre_ratings = torch.mul(bat_user_emd, bat_item_emd).sum(dim=1)
        loss1 = torch.nn.MSELoss()(pre_ratings,bat_ratings)

        bat_user_emd_p = self.user_embedding(bat_users)
        item_user_emd_p = self.item_embedding(bat_items)
        loss2 = self.reg_loss(bat_user_emd_p, item_user_emd_p)
        # print('mse_loss：{}，reg_loss{}'.format(loss1 , loss2 * self.reg_weight))
        return loss1  + loss2 * self.reg_weight
    ...
    def calc_ssl_loss(
        ...
        # print('ssl_loss:',(ssl_item + ssl_user) * self.ssl_weight)
    ...
    def predict(self,  bat_users,bat_items):
        ...
        user = self.restore_user_e[bat_users]
        item = self.restore_item_e[bat_items]
